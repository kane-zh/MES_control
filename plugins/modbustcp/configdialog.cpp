...
void ConfigDialog::showDataSource()
{
    ...
    QStringList list;
    list<<tr("索引")<<tr("名称")<<tr("使能")<<tr("说明")<<tr("主机")
         <<tr("端口号")<<tr("请求时间")<<tr("重复次数");
...
void ConfigDialog::setDataSource()
{
    ...
    if(ui->host->text()==""){
      QMessageBox::warning(this,tr("提示"),tr("主机名不能为空!!!"),QMessageBox::Yes);
      return;
    }
    if(ui->port->text()==""){
...
void ConfigDialog::getValueTest(){
    ...
    if(dataSourceInfor[dataSource].modbusDevice==nullptr){
        ...
        if (modbusDevice){
            modbusDevice->setConnectionParameter(QModbusDevice::NetworkAddressParameter,dataSourceInfor[dataSource].host);
            modbusDevice->setConnectionParameter(QModbusDevice::NetworkPortParameter, dataSourceInfor[dataSource].port);
    ...
    if(dataSourceInfor[dataSource].modbusDevice->state()==QModbusDevice::ConnectingState){
        QMessageBox::warning(this,tr("提示"),tr("请再次点击测试按钮!!"),QMessageBox::Yes);
        return ;
    }
    /*构建moubus读数据块*/
    ...
    QModbusDataUnit requestData=QModbusDataUnit(static_cast<QModbusDataUnit::RegisterType>(ui->code_read->currentData().toInt()),
                     ui->startAddr_read->text().toInt(),ui->length_read->text().toInt());
